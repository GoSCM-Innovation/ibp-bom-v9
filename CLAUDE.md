...
- Estilos: los objetos `style={{...}}` inline son la convención, pero los valores salen de `src/styles/`. Colores de la paleta: `color.*` (var CSS), `alpha.*(a)` (translúcido) o `hex.*` (cuando un helper tiene que derivar tintes o el valor va a un atributo SVG). Espaciado, tipografía y radio salen de las escalas `space`/`fontSize`/`radius`. Nunca escribir un `rgba()`/hex de la paleta a mano, ni concatenar el alfa al hex (`color + '22'`), ni usar un valor fuera de escala: hay tests que lo bloquean. Formularios, botones, estados de SAP, tipo de task y colores de avatar tienen módulo propio. Ver `docs/DESIGN-SYSTEM.md`.
