...
- **Columnas "Tipo de dato (IBP)" y "Ejemplo (IBP)"** en la hoja de detalle: el tipo sale de `types`; el ejemplo es un valor real. Se traen varias filas (`$top=50`) y por cada campo se toma el primer valor **no vacío**, para que el ejemplo no quede en blanco cuando la primera fila carece de valor.
